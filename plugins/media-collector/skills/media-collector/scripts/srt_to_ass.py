#!/usr/bin/env python3
\"\"\"
Universal SRT to Styled 1080p ASS Subtitle Converter
Usage: python3 srt_to_ass.py input.srt [output.ass] [--font "Noto Sans"] [--size 75]
\"\"\"
import sys
import os
...
Style: Default,{font_name},{font_size},&H00FFFFFF,&H00000000,&H00000000,&H00000000,0,0,0,0,100,100,0,0,1,4.5,0,2,20,20,50,1
...
\"\"\"
    ...
    for b in blocks:
        ...
        if len(lines) >= 3 and "-->" in lines[1]:
            ...
            dialogues.append(f"Dialogue: 0,{start},{end},Default,,0,0,0,,{dialogue_text}")
            
    with open(ass_path, "w", encoding="utf-8") as out:
        out.write(header + "\\n".join(dialogues) + "\\n")
    print(f"✅ Converted {len(dialogues)} lines -> {ass_path}")
