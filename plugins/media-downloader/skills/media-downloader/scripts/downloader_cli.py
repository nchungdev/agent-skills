...
def main():
    ...
    p_dl.add_argument("--out-dir", default="/Volumes/512GB/AI Workspace/media_staging", help="Thư mục lưu đệm")
    ...
    if args.command == "download":
        ...
        print(f"🚀 Khởi chạy Media Downloader [Provider: {prov.upper()}]...")

        if prov == "direct":
            success = download_direct(src, args.out_dir, connections=args.connections)
            sys.exit(0 if success else 1)
        elif prov == "aria2":
            success = download_aria2_torrent(src, args.out_dir, connections=args.connections)
            sys.exit(0 if success else 1)
        elif prov == "torbox":
            res = add_torrent(src)
