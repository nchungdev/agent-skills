...
```bash
# Khảo sát tương tác các tiêu chí (thể loại, thời lượng, chống seeding)
python3 advisor_cli.py setup

# Hoặc lưu nhanh cấu hình mặc định (khách quan, bao quát toàn bộ)
...
```
Hiển thị tổng quan: Hồ sơ khẩu vị (Taste profile), Phim đang xem dở, Kho báu bỏ quên trên NAS, và Phim chiếu rạp nổi bật.

### 3. Đề Xuất Phim Chưa Xem Trên NAS (Unwatched Gems)
```bash
python3 advisor_cli.py unwatched --limit 5
```

### 4. Tiếp Tục Theo Dõi (Continue Watching)
```bash
python3 advisor_cli.py continue --limit 5
```

### 5. Phim Đang Chiếu Rạp (Theatrical Releases)
```bash
python3 advisor_cli.py theatrical --limit 5
```

### 6. Xu Hướng Thịnh Hành Trực Tuyến (Trending OTT)
```bash
...
|:---:|---|
| `<img src="/path/to/poster_w185.jpg" width="115" alt="Title" />` | **Tên Phim** (Năm)<br>⭐ **Điểm số/10** (Số lượt đánh giá) · Huy hiệu kiểm định<br>📍 **Kho NAS**: 🟢 Sẵn sàng xem ngay / ⚪ Chưa có trên NAS<br>⏱️ **Tiến độ**: Đã xem X% (phút dở/tổng thời lượng)<br>📝 Tóm tắt nội dung... |
...
- Token TMDb được tái sử dụng từ `.agent/credentials.json` hoặc `~/.env`, tuyệt đối không in lộ ra báo cáo hay transcript.
